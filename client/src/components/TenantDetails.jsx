...
function TenantDetails(props) {
  const header = ["Tenant no", "Room_no", "Name", "Age", "DOB", "Starting Date", "Ending Date", "Stat", "Del"];
                ...
                <thead>
                  <tr className="bg-blue-500 text-center">
                    {header.map((ele, index) => (
                      <th
                        key={index + 1}
                        className="w-1/6 min-w-[120px] text-lg font-semibold text-white py-4 lg:py-7 px-3 lg:px-4 border-l border-transparent"
                      >
                        {ele}
                      </th>
                    ))}
                ...
                <tbody>
                  {tenantRows.map((ele, index) => (
                    <tr key={index + 1}>
                      ...
                      <td className="text-center text-dark font-medium text-base py-5 px-2 border-b border-l border-[#E8E8E8]">
                        {ele.starting_date.split('T')[0]}
                      </td>
                      <td className="text-center text-dark font-medium text-base py-5 px-2 border-b border-l border-[#E8E8E8]">
                        {ele.ending_date.split('T')[0]}
                      </td>
                      <td className="text-center text-dark font-medium text-base py-5 px-2 border-b border-l border-[#E8E8E8]">
                        {ele.stat === "Not-Paid" ? "Unpaid" : "Paid"}
                      </td>
                      <td className="text-dark font-semibold text-xl py-5 px-2 text-red-500 border-b border-l border-[#E8E8E8] text-center">
                        <MdDeleteForever className="cursor-pointer" onClick={() => deleteTenant(ele.tenant_id)} />
                      </td>
                    </tr>
                  ))}
                </tbody>
